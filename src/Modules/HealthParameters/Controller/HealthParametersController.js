...
import { QUERY_UPDATE_PATIENT_VITALS } from "../../../GraphQL/Mutation"
import { QUERY_GET_PATIENT_HEALTH_PROFILE } from "../../../GraphQL/Queries"
...
export const savePatientHealthParameters = async (data) => {
    ...
    try {
        const result = await clientAuth
            .mutate({
                mutation: QUERY_UPDATE_PATIENT_VITALS,
...
export const getPatientHealthProfile = async (data) => {
    let response = {};
    try {
        const result = await clientAuth.query({
            query: QUERY_GET_PATIENT_HEALTH_PROFILE,
            variables: { id: data.id },
            fetchPolicy: 'no-cache',
        });
        response = result?.data || {};
    } catch (err) {
        console.error("Query error (getPatientHealthProfile):", err);
        return err;
    }
    return response;
};
